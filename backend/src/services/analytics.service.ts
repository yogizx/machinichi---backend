...
export const trackView = async (
  ...
  const cleanPid = typeof productId === 'string' ? productId.trim() : '';
  if (!cleanPid || !Types.ObjectId.isValid(cleanPid)) {
    return null;
  }
  const pid = new Types.ObjectId(cleanPid);
  const cleanUid = identity.userId && Types.ObjectId.isValid(identity.userId.trim()) ? identity.userId.trim() : undefined;
  const uid = cleanUid;
  ...
  try {
    ...
    if (err?.code === 11000) {
      await View.findOneAndUpdate(query, { lastViewedAt: new Date() }).catch(() => {});
    }
  }
  await recordViewEvent(pid, uid, gid, sid, meta).catch(() => {});
