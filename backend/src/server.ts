...
app.use((req: Request, _res: Response, next: NextFunction) => {
  const sanitize = (obj: any): any => {
    ...
    for (const [key, value] of Object.entries(obj)) {
      // Prevent dollar-prefixed NoSQL operator injection from client payloads
      if (key.startsWith('$')) continue;
      sanitized[key] = typeof value === 'object' && value !== null ? sanitize(value) : value;
    }
    return sanitized;
  };
...
app.use((err: any, _req: any, res: any, _next: any) => {
  ...
  if (err?.code === 11000) {
    ...
    const val = err?.keyValue ? JSON.stringify(err.keyValue) : '';
    return res.status(409).json({ success: false, message: `Duplicate entry for ${field} ${val}`.trim() });
  }
...
async function connectWithRetry(retries = 5, delayMs = 3000) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      console.log('[SERVER MONGO URI]', (MONGO_URI as string).replace(/:([^:@]+)@/, ':****@'));
